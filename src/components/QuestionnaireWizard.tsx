    ...
    isMarketingRestricted ? null : !hasWebsite ? (
      ...
      <div className="flex flex-col gap-1">
        <button
          ...
          disabled={suggesting}
          className="inline-flex items-center gap-1.5 text-xs text-teal-600 hover:text-teal-700 font-medium cursor-pointer disabled:opacity-50 self-start"
        >
          {suggesting ? (
            <>
            ...
            <>
              ...
              {suggestError ? "Try again" : "Write with AI"}
            </>
          )}
        </button>
        {suggestError && (
          <p className="text-xs text-red-500">AI is temporarily busy — please try again in a moment.</p>
        )}
      </div>
    )
  );
